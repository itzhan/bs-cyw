...
CREATE TABLE control_strategy (
    ...
    start_datetime DATETIME COMMENT '精确开始时刻(年月日时分)',
    end_datetime DATETIME COMMENT '精确结束时刻(年月日时分)',
    last_phase TINYINT DEFAULT 0 COMMENT '调度阶段: 0-未开始 1-执行中 2-已结束',
    status TINYINT NOT NULL DEFAULT 1 COMMENT '状态: 0-禁用 1-启用',
...
CREATE TABLE work_order (
    ...
    repair_report_id BIGINT COMMENT '关联报修ID(审核通过自动生成)',
    streetlight_id BIGINT COMMENT '关联路灯ID',
    area_id BIGINT COMMENT '所属区域ID',
